...
# Prefer the shield-trade-loop Hardhat project in this repo
if [ -f "${SCRIPT_DIR}/../../hardhat.config.ts" ] || [ -f "${SCRIPT_DIR}/../../hardhat.config.js" ]; then
  cd "${SCRIPT_DIR}/../../" # -> shield-trade-loop
elif [ -d "${SCRIPT_DIR}/../../fhevm-hardhat-template" ]; then
  cd "${SCRIPT_DIR}/../.."
else
  echo "Could not locate a Hardhat project relative to ${SCRIPT_DIR}." >&2
  exit 1
fi
